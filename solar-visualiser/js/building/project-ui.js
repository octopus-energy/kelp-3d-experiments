window.SolarViz.setupProject=function({building}){
 ...
 function render(){
  ...
  if(report&&report.propertyId===SV.currentProperty.id){
   const proposal=document.createElement('a');proposal.id='bm-proposal';proposal.href='proposal.html?working=1';proposal.className='btn';proposal.textContent='Your installation · pre-survey discussion →';box.appendChild(proposal);
   const replay=document.createElement('a');replay.id='bm-replay';replay.href='replay.html';replay.className='btn';replay.textContent='Watch reconstruction replay →';box.appendChild(replay);
