...
function validateObservation(o,data){
 if(!o||!['emitter','envelope','basement','geometry','access','photo-room','comfort','hot-water','radiator-evidence','discussion','room-context','service','planning-check','envelope-uncertain','surface-assumption','radiator-inventory','placement'].includes(o.kind)||!['surveyor','homeowner','adviser'].includes(o.role)||['observer','note','at','id'].some(k=>typeof o[k]!=='string'||!o[k].trim())||!Number.isFinite(Date.parse(o.at)))throw Error('Record who checked it and the evidence / measurement source.');
 K.validateObservation(o,data);
 if(o.kind==='placement'){if(!['heatPump','cylinder','radiator'].includes(o.target))throw Error('Choose equipment to place.');const X=typeof module==='object'&&module.exports?require('./spatial'):SolarViz.spatial;if(o.position!==null)X.validatePosition(o.position,data,{indoor:o.target==='cylinder',outdoor:o.target==='heatPump',roomId:o.target==='radiator'?o.roomId:undefined});if(o.target==='radiator'&&(!o.radiatorId||!data.geometry.geometry.rooms.some(r=>r.id===o.roomId)))throw Error('Choose a radiator and room.');}
...
function packageFor(data,project,flow){
 ...
 const rooms=r.rooms.map(room=>{
  ...
  const laterEvidence=project.observations.slice(observedIndex+1).some(o=>{
   if(o.kind==='placement')return o.roomId===room.id;
 ...
 const blocked=project.household.noKitchenCylinder&&r.routing.cylinderRoomId===data.geometry.geometry.rooms.find(r=>r.sourceRoomId==='g-kitchen').id;
...
function observe(project,data,observation){
 ...
 if(o.kind==='placement'){choices.spatial={...choices.spatial};if(o.target==='radiator'){const room=data.geometry.geometry.rooms.find(r=>r.id===o.roomId);if(!A.seedInventory(data,choices,room).items.some(i=>i.id===o.radiatorId))throw Error('Choose a listed radiator.');choices.spatial.radiators={...choices.spatial.radiators};const key=o.roomId+'|'+o.radiatorId;if(o.position===null)delete choices.spatial.radiators[key];else choices.spatial.radiators[key]={roomId:o.roomId,radiatorId:o.radiatorId,position:o.position};}else if(o.position===null)delete choices.spatial[o.target];else choices.spatial[o.target]=o.position;}
...
function tasks(data,project){
 ...
 if(project.household.planning?.paths?.length)out.push({id:'walking-paths',kind:'access',target:'courtyard',title:'Keep your marked paths usable',owner:'Surveyor / homeowner',impactW:null,why:project.household.planning.paths.length+' walking routes marked on the shared plan. Widths, doors, gates and garden levels still need checking.',method:'Walk each marked route together. Check clear width, doors, bins, service access and any crossing pipework; agree changes on the equipment plan.'});
