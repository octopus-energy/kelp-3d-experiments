window.SolarViz.createProposalView=function(data){
 ...
 function update(result,chapter,{showServices=chapter===2||chapter===3||chapter===4}={}){
  ...
  if(show){const r=result.routing,hp=new T.Mesh(new T.BoxGeometry(1.1,.95,.45),mat(0xf4f5ee));hp.position.set(...r.hp);services.add(hp);const fan=new T.Mesh(new T.CylinderGeometry(.34,.34,.02,32),mat(0x52675e));fan.rotation.x=Math.PI/2;fan.position.set(r.hp[0],r.hp[1],r.hp[2]+.235);services.add(fan);label('Outdoor unit · option',[r.hp[0]+.8,r.hp[1]+1.45,r.hp[2]]);
   if(r.cylinder){const cy=new T.Mesh(new T.CylinderGeometry(.35,.35,1.5,32),mat(0x58a2b5));cy.position.set(...r.cylinder);services.add(cy);label('Cylinder · proposed space',[r.cylinder[0],r.cylinder[1]+1.6,r.cylinder[2]]);
