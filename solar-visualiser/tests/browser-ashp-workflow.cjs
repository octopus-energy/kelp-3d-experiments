...
(async()=>{
 ...
 for(const root of [process.env.REVIEW_HTTP_URL,pathToFileURL(appRoot).href]){
  ...
  assert.equal(await evalJS('__SOLAR_VIZ__.building.state.geometrySource.stageId'),'exterior-refined');
  ...
  assert.equal(await evalJS('__SOLAR_VIZ__.building.state.windows.length+__SOLAR_VIZ__.building.state.unboundCandidateOpenings.length'),14);
