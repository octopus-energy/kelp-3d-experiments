...
(async()=>{
 ...
 for(const root of [process.env.REVIEW_HTTP_URL,pathToFileURL(appRoot).href]){
  ...
  assert.equal(await evalJS('document.querySelectorAll(".element-breakdown button").length'),7);
  assert.equal(await evalJS('!!window.__ROOM_ASSESSMENT_VIEW__&&__ROOM_ASSESSMENT_VIEW__.meshes.length>10'),true);
  const boundaryBefore=await evalJS('__BROOM_INSTALLATION__.brief.selected.loadW');
  assert.equal(await evalJS('document.querySelector("[data-assessment-form=boundary]").elements.note.required'),false);
  await fill('[data-assessment-form=boundary]',{value:'unheated',note:''});
  assert(await evalJS('__BROOM_INSTALLATION__.brief.selected.loadW')<boundaryBefore,'A homeowner answer saves and changes the assumption without a justification');
  assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.at(-1).note'),'Homeowner reports: An unheated space, such as a garage');
  assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.at(-1).role'),'homeowner');
  assert.equal(await evalJS('document.querySelector("[data-assessment-form=boundary]").elements.value.value'),'unheated','The saved answer stays selected');
  await snapshot('boundary-simple-desktop','.boundary-assumption');
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:2,mobile:true});await snapshot('boundary-simple-phone','.boundary-assumption');
  ...
  await fill('[data-assessment-form=boundary]',{value:'uncertain',note:''});
  assert.equal(await evalJS('__BROOM_INSTALLATION__.brief.selected.loadW'),boundaryBefore,'Not sure reopens the question without demanding an explanation');
  await evalJS(`document.querySelector('header [data-role=adviser]').click();__BROOM_INSTALLATION__.go(2)`);
  assert.equal(await evalJS('document.querySelector("[data-assessment-form=boundary]").elements.note.required'),true,'Professional assessment retains its evidence requirement');
  const professionalCount=await evalJS('__BROOM_INSTALLATION__.project.observations.length');await fill('[data-assessment-form=surface]',{u:'2.1',note:'   '});assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.length'),professionalCount,'Whitespace cannot turn a professional assessment into a homeowner report');await evalJS('document.querySelector("[data-assessment-form=surface]").reset()');
  await evalJS(`document.querySelector('header [data-role=customer]').click();__BROOM_INSTALLATION__.go(2)`);
  ...
  await fill('[data-assessment-form=surface]',{u:'4.8',note:''});
  ...
  assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.at(-1).note.includes("supporting source not supplied")'),true,'Optional explanation never fabricates a measurement source');
