...
(async()=>{
 ...
 for(const root of [process.env.REVIEW_HTTP_URL,pathToFileURL(appRoot).href]){
  ...
  await evalJS(`document.querySelector('[data-element-category=floors]').click();document.querySelector('[data-home-construction-note]').focus()`);await call('Input.insertText',{text:'Synthetic floor insulation account'});
  const next=await evalJS(`(()=>{const b=document.getElementById('room-guide-next');b.scrollIntoView({block:'center'});const r=b.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};})()`);await call('Input.dispatchMouseEvent',{type:'mousePressed',...next,button:'left',clickCount:1});await call('Input.dispatchMouseEvent',{type:'mouseReleased',...next,button:'left',clickCount:1});assert.equal(await evalJS('document.getElementById("room-jump").value'),'0:r10','A note saves and next room advances in one click');assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.at(-1).note.includes("Synthetic floor insulation account")'),true);await evalJS(`document.getElementById('room-jump').value='0:r0';document.getElementById('room-jump').dispatchEvent(new Event('change'))`);
