...
(async()=>{
 ...
 for(const root of [process.env.REVIEW_HTTP_URL,pathToFileURL(appRoot).href]){
  ...
  await evalJS(`__BROOM_INSTALLATION__.go(0);document.querySelector('[data-priority="running"]').click();document.getElementById('no-kitchen').click();document.querySelector('[data-preserve="0:r11"]').click()`);
  ...
  await evalJS(`__BROOM_INSTALLATION__.go(0);document.getElementById('comfort-choices').open=true;document.querySelector('[data-cold="1:r01"]').click()`);
  ...
  await evalJS(`document.querySelector('[data-residents="4"]').click();document.querySelector('[data-habit="morning"]').click();document.querySelector('[data-habit="overlap"]').click()`);
  ...
  await evalJS(`document.getElementById('photo-matching').open=true;const i=BROOM_PROPOSAL.interiorPhotos.findIndex(i=>i.id==='8d1cee4f0ff1f1632f03ce295d77e9eb');document.querySelector('[data-photo-index="'+i+'"]').click();document.querySelector('[data-match-room="f-bed2"]').click();document.getElementById('match-photo').click()`);
  ...
  await evalJS(`document.getElementById('matching-evidence').scrollIntoView({block:'start'})`);await new Promise(r=>setTimeout(r,150));
  ...
  await evalJS(`document.querySelector('[data-match-room="f-bed3"]').click();document.getElementById('match-photo').click();__BROOM_INSTALLATION__.go(5)`);
  ...
  await evalJS(`__BROOM_INSTALLATION__.go(0);document.getElementById('comfort-choices').open=true;document.getElementById('photo-matching').open=true`);assert.equal(await evalJS('document.documentElement.scrollWidth<=innerWidth'),true,'Expanded household controls fit phone');fs.writeFileSync(path.join(testTmp,'installation-phone.png'),Buffer.from((await call('Page.captureScreenshot',{format:'png'})).data,'base64'));
  await evalJS(`document.getElementById('comfort-choices').scrollIntoView()`);await new Promise(r=>setTimeout(r,150));fs.writeFileSync(path.join(testTmp,'installation-room-cards-phone.png'),Buffer.from((await call('Page.captureScreenshot',{format:'png'})).data,'base64'));
