...
(async()=>{
 ...
 for(const root of [process.env.REVIEW_HTTP_URL,pathToFileURL(appRoot).href]){
  ...
  const hit=await evalJS(`(()=>{const v=__BROOM_INSTALLATION__.view,m=v.services.children.find(m=>m.userData.markerId==='proposed:cylinder');v.renderer.domElement.scrollIntoView({block:'center'});const p=m.getWorldPosition(new THREE.Vector3()).project(v.camera),b=v.renderer.domElement.getBoundingClientRect();return {x:b.x+(p.x+1)*b.width/2,y:b.y+(1-p.y)*b.height/2};})()`);await call('Input.dispatchMouseEvent',{type:'mousePressed',...hit,button:'left',clickCount:1});await call('Input.dispatchMouseEvent',{type:'mouseReleased',...hit,button:'left',clickCount:1});assert.equal(await evalJS(`document.querySelector('[data-spatial-target="proposed:cylinder"]').getAttribute('aria-pressed')`),'true','3D equipment selects the matching plan item');
  ...
  await evalJS('__BROOM_INSTALLATION__.go(3)');await click('[data-spatial-target="service:meter"]');await click('[data-plan-panel="equipment-plan"] [data-floor-choice="0"]');await click('[data-start-placement]');await planPoint('#equipment-plan',kitchen[0]-.4,kitchen[1]);assert.equal(await evalJS('__BROOM_INSTALLATION__.project.observations.at(-1).target'),'meter');assert.equal(await evalJS(`document.querySelector('#equipment-plan [data-plan-item]:last-of-type')?.dataset.planItem==='service:meter'||[...document.querySelectorAll('#equipment-plan [data-plan-item]')].at(-1).dataset.planItem==='service:meter'`),true,'Selected item stays visible above overlapping marks');await click('[data-plan-panel="equipment-plan"] [data-plan-zoom="house"]');await snapshot('phone-placed-map','#equipment-plan');
  await click('[data-service-unknown]');assert.equal(await evalJS(`SolarViz.spatial.markers(BROOM_PROPOSAL,__BROOM_INSTALLATION__.project).some(m=>m.id==='service:meter')`),false,'Unknown does not imply an equipment position');
