...
const recapture=J.observe(revised,D,{...capture.observations[0],id:'after-inventory'});
const reopenedRoom=J.packageFor(D,recapture,50).rooms.find(r=>r.id===living.id);
assert.equal(reopenedRoom.inventoryVerified,false);
assert.equal(reopenedRoom.inventoryNeedsReview,true);
assert.equal(J.roomGuide(reopenedRoom).status,'Inventory needs review');
assert(J.tasks(D,recapture).some(t=>t.id==='emitter-'+living.id));
const reassessed=J.observe(recapture,D,{...o,id:'inventory-after-photos'});
assert.equal(J.packageFor(D,reassessed,50).rooms.find(r=>r.id===living.id).inventoryVerified,true);
assert(!J.tasks(D,reassessed).some(t=>t.id==='review-photos-'+living.id));
assert.equal(J.replay(reassessed,D,2).observations.length,2);
assert.equal(J.packageFor(D,J.replay(reassessed,D,2),50).rooms.find(r=>r.id===living.id).inventoryVerified,false);
const sourcePhoto=living.sourceEvidence[0].imageId;
const reaffirmed=J.assignPhoto(revised,D,sourcePhoto,living.sourceRoomId);
assert.equal(J.packageFor(D,reaffirmed,50).rooms.find(r=>r.id===living.id).inventoryVerified,true,'Confirming unchanged room identity retains inventory');
const identical=J.assignPhoto(reaffirmed,D,sourcePhoto,living.sourceRoomId);
assert.equal(identical.events.length,reaffirmed.events.length,'Saved identical matches create no duplicate evidence');
assert.equal(J.assignPhoto(baseline,D,image,'f-bed2','surveyor').observations[0].role,'surveyor');
const reported=J.observe(baseline,D,{...o,role:'homeowner'});
assert.equal(J.roomGuide(J.packageFor(D,reported,50).rooms.find(r=>r.id===living.id)).status,'Reported rating · to verify');
assert.equal(J.roomGuide(absentRoom).approach,'Choose heating for this room');
for(const household of [{...baseline.household,roomDesign:{[living.id]:null}},{...baseline.household,roomFeedback:[]},{...baseline.household,sitePreferences:{}},{...baseline.household,preserve:[living.id,living.id]}])assert.throws(()=>J.validate({...baseline,household},D),/Invalid/);
const badHistory=structuredClone(revised);badHistory.events[0].beforeState.household.noKitchenCylinder='false';assert.throws(()=>J.validate(badHistory,D),/Invalid household/);
assert(J.householdBrief({...baseline,choices:{...baseline.choices,preferences:{...baseline.choices.preferences,occupants:'1'}}}).includes('1 person lives'));
console.log('Inventory evidence ordering, assessment, attribution and replay validation passed');
