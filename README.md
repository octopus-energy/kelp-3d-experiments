...
- **Facade projection**: with matched photos, "Project photos onto walls" bakes them onto the wall panels — every texel back-projected through the poses with occlusion handling, so the model wears the real brickwork to scale. Click a wall to open the rectified facade editor and drag rectangles over the visible windows and doors: the photo does the measuring, and each rectangle becomes a real opening (hole + glass) in the model
