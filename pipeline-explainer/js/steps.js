...
<p>Generation is then the MCS formula: <code>kWp &times; kWh/kWp &times; SF</code>, where kWh/kWp comes from the regional irradiance table for the face's azimuth and pitch, with monthly shape from the SAP tables. Summing every accepted array gives the property total: <strong>${data.siteData.total_annual_generation_kwh.toLocaleString()} kWh/year</strong> from ${data.siteData.total_number_of_panels} panels.</p>`,
    ...
    // ---------------- STEPS 7-9: the MPPT mini-game ----------------
    {
      title: '7 · One string, one dial',
      text: `
<p>The design is done — now the electricity. Panels are wired in <strong>series</strong> into a <strong>string</strong> (follow the coloured wires to the inverter): their voltages add up, and one shared current flows through every panel.</p>
<p>Here's the unintuitive part: a string has <strong>no fixed output</strong>. The inverter must <em>choose</em> the voltage to run it at — and the power depends entirely on that choice. The chart shows every possible choice at one frozen moment of the morning.</p>
<p><strong>Try it:</strong> drag on the chart and watch the wattage. Too low and you leave power on the table; too high and the current collapses. Somewhere in between…</p>`,
      code: `illustrative electrical layer — not in solar_potential_from_address (yet)`,
      enter() {
        enterGameStep('spot', 7);
      },
    },

    {
      title: '8 · Meet the MPPT',
      text: `
<p>The spot you found has a name: the <strong>Maximum Power Point</strong>. And it never sits still — sun angle, clouds and temperature drag it around from minute to minute.</p>
<p>The machine that chases it is the <strong>MPPT</strong> — Maximum Power Point <em>Tracker</em> — a small controller in the inverter that endlessly nudges the voltage, re-measures, and corrects, hundreds of times a second. This inverter has <strong>three independent trackers</strong>, each running its own string at its own voltage.</p>
<p><strong>Your shift:</strong> press play and do its job by hand for one day. The grey dot is the perfect tracker — the score compares your harvest against it.</p>`,
      code: `illustrative electrical layer — not in solar_potential_from_address (yet)`,
      enter() {
        enterGameStep('track', 8);
      },
    },

    {
      title: '9 · One orientation per tracker',
      text: `
<p>Now the trap. Wire <strong>north- and south-facing panels into one string</strong> and they still must share one current. Whichever side the sun favours, the other side can't carry its current — each starved panel's <strong>bypass diode</strong> cuts it out of the circuit, and the P–V curve splits into <strong>two humps</strong>.</p>
<p>A tracker — you, or the real thing — can only stand on <em>one</em> hump at a time, and the taller hump switches sides during the day. Play the day and feel it; use the wiring toggle to compare.</p>
<p>Across the whole system this wiring costs real energy — which is why the design rule is simple: <strong>one roof orientation per tracker input</strong>.</p>`,
      code: `illustrative electrical layer — not in solar_potential_from_address (yet)`,
      enter() {
        enterGameStep('mixed', 9);
      },
    },
  ];

  // shared bootstrap for the three MPPT-game steps: wiring worlds, sun
  // disc, dimmed-backdrop camera, then hand over to the game module
  function enterGameStep(stage, stepIndex) {
    if (!world.elec) {
      world.elec = {
    ...
    const centre = viz.toScene(bc[0], bc[1], ground + 4);
    applyState({
      panels3D: 0.55, morph: 1, electrical: 1,
      camera: { pos: new THREE.Vector3(centre.x - 30, ground + 46, centre.z + 42), tgt: centre, duration: 1600 },
    });
    viz.addLabel('inverter · 3.68 kW · 3 MPPT', world.elec.mixed.invPos.clone().add(new THREE.Vector3(0, 1.3, 0)), { cls: 'dim' });
    PE.mpptGame.show({ data, viz, world, ctx }, stage, stepIndex);
  }
